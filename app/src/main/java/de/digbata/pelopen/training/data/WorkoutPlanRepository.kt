...
/**
 * Repository for fetching and storing training plans.
 * It fetches from an online service and caches them locally.
 */
class WorkoutPlanRepository(private val context: Context) {
    ...
    fun savePlan(plan: WorkoutPlan, force: Boolean = false) {
        val planFile = File(plansDir, "${plan.workoutId}.json")
        // Avoid rewriting if the plan already exists
        if (!force && planFile.exists()) return
    ...
    fun loadPlan(workoutId: String): WorkoutPlan? {
        ...
        return try {
            ...
            Timber.e(e, "Failed to load/parse workout plan ${workoutId} from cache")
            null
        }
    }
}